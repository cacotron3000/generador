    ...
    switch ($method) {
        ...
        case 'GET':
            if (isset($_GET['uuid'])) {
                ...
                $st = $pdo->prepare("SELECT uuid, nombre, suma, cuerpo, petitorio, usa_delegados, creado_en, actualizado_en, eliminado_en FROM escritos_biblioteca WHERE uuid = ?");
                $st->execute([$uuid]);
                $row = $st->fetch();
            ...
            if (isset($_GET['since'])) {
                ...
                $st = $pdo->prepare("SELECT uuid, nombre, suma, cuerpo, petitorio, usa_delegados, creado_en, actualizado_en, eliminado_en FROM escritos_biblioteca WHERE actualizado_en >= ? ORDER BY actualizado_en ASC");
                $st->execute([$since]);
                $rows = array_map('format_row', $st->fetchAll());
                json_response(200, ['escritos' => $rows, 'server_time' => fmt_datetime_utc(gmdate('Y-m-d H:i:s.') . '000')]);
            }
            $rows = $pdo->query("SELECT uuid, nombre, suma, cuerpo, petitorio, usa_delegados, creado_en, actualizado_en, eliminado_en FROM escritos_biblioteca WHERE eliminado_en IS NULL ORDER BY actualizado_en DESC")->fetchAll();
        ...
        case 'POST':
            ...
            $st = $pdo->prepare("
                INSERT INTO escritos_biblioteca (uuid, nombre, suma, cuerpo, petitorio, usa_delegados)
                VALUES (?, ?, ?, ?, ?, ?)
                ON DUPLICATE KEY UPDATE
                  ...
                  eliminado_en = NULL
            ");
            $st->execute([$uuid, $nombre, $suma, $cuerpo, $petitorio, $usaDelegados]);

            $st2 = $pdo->prepare("SELECT uuid, nombre, suma, cuerpo, petitorio, usa_delegados, creado_en, actualizado_en, eliminado_en FROM escritos_biblioteca WHERE uuid = ?");
            $st2->execute([$uuid]);
