    ...
    switch ($method) {
        ...
        case 'GET':
            if (isset($_GET['uuid'])) {
                ...
                $st = $pdo->prepare("SELECT uuid, titulo, nombre, contenido, cuerpo, creado_en, actualizado_en, eliminado_en FROM escritos_biblioteca WHERE uuid = ?");
                $st->execute([$uuid]);
                $row = $st->fetch();
            ...
            if (isset($_GET['since'])) {
                ...
                $st = $pdo->prepare("SELECT uuid, titulo, nombre, contenido, cuerpo, creado_en, actualizado_en, eliminado_en FROM escritos_biblioteca WHERE actualizado_en >= ? ORDER BY actualizado_en ASC");
                $st->execute([$since]);
                $rows = array_map('format_row', $st->fetchAll());
                json_response(200, ['otrosies' => $rows, 'server_time' => fmt_datetime_utc(gmdate('Y-m-d H:i:s.') . '000')]);
            }
            $rows = $pdo->query("SELECT uuid, titulo, nombre, contenido, cuerpo, creado_en, actualizado_en, eliminado_en FROM escritos_biblioteca WHERE eliminado_en IS NULL ORDER BY actualizado_en DESC")->fetchAll();
        ...
        case 'POST':
            ...
            $st = $pdo->prepare("
                INSERT INTO escritos_biblioteca (uuid, titulo, nombre, contenido, cuerpo, suma, petitorio)
                VALUES (?, ?, ?, ?, ?, '', '')
                ON DUPLICATE KEY UPDATE
                  titulo = VALUES(titulo),
                  nombre = CASE WHEN VALUES(nombre) <> '' THEN VALUES(nombre) ELSE nombre END,
                  contenido = VALUES(contenido),
                  cuerpo = CASE WHEN VALUES(cuerpo) <> '' THEN VALUES(cuerpo) ELSE cuerpo END,
                  eliminado_en = NULL
            ");
            $st->execute([$uuid, $titulo, $titulo, $contenido, $contenido]);

            $st2 = $pdo->prepare("SELECT uuid, titulo, nombre, contenido, cuerpo, creado_en, actualizado_en, eliminado_en FROM escritos_biblioteca WHERE uuid = ?");
            $st2->execute([$uuid]);
        ...
        case 'PUT':
            ...
            if (isset($data['contenido'])) {
                ...
                $sets[] = 'cuerpo = CASE WHEN ? <> "" THEN ? ELSE cuerpo END';
                $args[] = $contenido; $args[] = $contenido;
            }
...
function format_row(array $row): array
{
    return [
        ...
        'contenido'     => ($row['contenido'] ?? '') !== '' ? $row['contenido'] : ($row['cuerpo'] ?? ''),
